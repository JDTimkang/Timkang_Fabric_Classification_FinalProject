...
/// Fabric classifier backed by a TensorFlow Lite model.
///
/// Loads `assets/model_unquant.tflite` and `assets/labels.txt` and exposes
/// helper methods for classifying files, camera frames and bundled assets.
class FabricClassifierService {
  FabricClassifierService._();

  static final FabricClassifierService instance = FabricClassifierService._();
  ...
  /// Classify an image bundled as an asset (e.g. `assets/images/foo.png`).
  /// This is useful for demo / CI scenarios where a CameraImage is not available.
  Future<ClassificationResult?> classifyAsset(String assetPath) async {
    await ensureModelLoaded();
